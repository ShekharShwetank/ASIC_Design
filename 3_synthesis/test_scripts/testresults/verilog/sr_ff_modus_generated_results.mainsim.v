...
//***************************************************************************//
//                                                                           //
//  FILE CREATED..............September 09, 2025 at 17:41:19                 //
//                                                                           //
...
//***************************************************************************//
  ...
  sr_ff
    sr_ff_inst (
      ...
      .Q_bar        ( part_POs [2] ),      // pinName = Q_bar; 
      .scan_out     ( part_POs [3] ),      // pinName = scan_out;  tf =  SO  ; 
      .R            ( part_PIs [1] ),      // pinName = R; testOffset = 0.000000;  scanOffset = 0.000000;  
      .S            ( part_PIs [2] ),      // pinName = S; testOffset = 0.000000;  scanOffset = 0.000000;  
      .SE           ( part_PIs [3] ),      // pinName = SE;  tf = +SE  ; testOffset = 0.000000;  scanOffset = 0.000000;  
...
//***************************************************************************//
  ...
  assign ( weak0, weak1 ) // Termination 
    ...
    part_POs [2] = global_term,     // pinName = Q_bar; 
...
//***************************************************************************//
  ...
  initial 
    begin 
      ...
      `ifdef sdf_annotate 
        `ifdef SDF_Minimum 
          $sdf_annotate ("default.sdf",sr_ff_inst,,"sdf_Min.log","MINIMUM");
        `endif 
        `ifdef SDF_Maximum 
          $sdf_annotate ("default.sdf",sr_ff_inst,,"sdf_Max.log","MAXIMUM");
        `endif 
        `ifdef SDF_Typical
          $sdf_annotate ("default.sdf",sr_ff_inst,,"sdf_Typ.log","TYPICAL");
        `endif 
      `endif 
...
//***************************************************************************//
  ...
  task sim_setup; 
    begin 
      ...
      name_POs [2] = "Q_bar";      // pinName = Q_bar; 
...
//***************************************************************************//
  ...
  task sim_vector_file; 
    begin 
      ...
      $display ( "\nINFO (TVE-189): Design:  sr_ff   Test Mode:  FULLSCAN   InExperiment:  logic " ); 
  ...
  task test_cycle; 
    begin 
     ...
     #0.000000;        // 0.000000 ns;  From the start of the cycle.
      part_PIs [1] = stim_PIs [1];      // pinName = R; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [2] = stim_PIs [2];      // pinName = S; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [3] = stim_PIs [3];      // pinName = SE;  tf = +SE  ; testOffset = 0.000000;  scanOffset = 0.000000;  
     ...
     #56.000000;        // 72.000000 ns;  From the start of the cycle.
      ...
      for ( POnum = 1; POnum <= 3; POnum = POnum + 1 ) begin 
        if (( part_POs [ POnum ] !== meas_POs [ POnum ] ) & ( meas_POs [ POnum ] !== 1'bX )) begin 
          ...
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "sr_ff", name_POs [ POnum ], PATTERN, -1, part_POs [ POnum ] ); 
          end  
        end  
...
//***************************************************************************//
  ...
  task Scan_Preconditioning_Sequence_TM_1_SEQ_1_SOP_1; 
    begin 
      ...
      stim_PIs [3] = 1'b1;      // pinName = SE;  tf = +SE  ; testOffset = 0.000000;  scanOffset = 0.000000;  
    ...
    begin 
     ...
     #8.000000;        // 8.000000 ns;  From the start of the cycle.
        ...
        if (( part_POs [3] !== meas_OR_1 [ 0 + SCANCYCLE ] ) & ( meas_OR_1 [ 0 + SCANCYCLE ] !== 1'bX )) begin      // pinName = scan_out;  tf =  SO  ; 
          ...
          if (( failset ) & ( FAILSETID == 0 ))  failset_setup; 
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "sr_ff", name_POs [3], PATTERN, SCANCYCLE, part_POs [3] ); 
          end  
        end  
        else  begin 
      ...
      case ( CMD ) 
        ...
        904: begin 
          ...
          if ( rc_read > 0 )  begin 
            `ifdef MISCOMPAREDEBUG 
              if ( diag_debug ) begin 
                $processSimulationDebugFile ( DIAG_DEBUG_FILE, "sr_ff_inst", eventID ); 
              end 
            `endif 
          end  
          else  bad_cmd_code; 
