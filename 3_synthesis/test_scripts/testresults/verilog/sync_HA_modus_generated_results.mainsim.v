...
//***************************************************************************//
//                                                                           //
//  FILE CREATED..............September 09, 2025 at 16:41:05                 //
//                                                                           //
...
//***************************************************************************//
  ...
  sync_HA
    sync_HA_inst (
      .c_out        ( part_POs [1] ),      // pinName = c_out; 
...
//***************************************************************************//
  ...
  assign ( weak0, weak1 ) // Termination 
    part_POs [1] = global_term,     // pinName = c_out; 
...
//***************************************************************************//
  ...
  initial 
    begin 
      ...
      `ifdef sdf_annotate 
        `ifdef SDF_Minimum 
          $sdf_annotate ("default.sdf",sync_HA_inst,,"sdf_Min.log","MINIMUM");
        `endif 
        `ifdef SDF_Maximum 
          $sdf_annotate ("default.sdf",sync_HA_inst,,"sdf_Max.log","MAXIMUM");
        `endif 
        `ifdef SDF_Typical
          $sdf_annotate ("default.sdf",sync_HA_inst,,"sdf_Typ.log","TYPICAL");
        `endif 
      `endif 
...
//***************************************************************************//
  ...
  task sim_setup; 
    begin 
      ...
      SERIALCYCLE = 0; 
      SEQNUM = 0; 
      name_POs [1] = "c_out";      // pinName = c_out; 
...
//***************************************************************************//
  ...
  task sim_vector_file; 
    begin 
      ...
      $display ( "\nINFO (TVE-189): Design:  sync_HA   Test Mode:  FULLSCAN   InExperiment:  logic " ); 
     ...
     #56.000000;        // 72.000000 ns;  From the start of the cycle.
      ...
      for ( POnum = 1; POnum <= 3; POnum = POnum + 1 ) begin 
        if (( part_POs [ POnum ] !== meas_POs [ POnum ] ) & ( meas_POs [ POnum ] !== 1'bX )) begin 
          ...
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "sync_HA", name_POs [ POnum ], PATTERN, -1, part_POs [ POnum ] ); 
          end  
        end  
    ...
    begin 
     ...
     #8.000000;        // 8.000000 ns;  From the start of the cycle.
        ...
        if (( part_POs [2] !== meas_OR_1 [ 0 + SCANCYCLE ] ) & ( meas_OR_1 [ 0 + SCANCYCLE ] !== 1'bX )) begin      // pinName = scan_out;  tf =  SO  ; 
          ...
          if (( failset ) & ( FAILSETID == 0 ))  failset_setup; 
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "sync_HA", name_POs [2], PATTERN, SCANCYCLE, part_POs [2] ); 
          end  
        end  
        else  begin 
      ...
      case ( CMD ) 
        ...
        904: begin 
          ...
          if ( rc_read > 0 )  begin 
            `ifdef MISCOMPAREDEBUG 
              if ( diag_debug ) begin 
                $processSimulationDebugFile ( DIAG_DEBUG_FILE, "sync_HA_inst", eventID ); 
              end 
            `endif 
          end  
          else  bad_cmd_code; 
