...
//***************************************************************************//
//                                                                           //
//  FILE CREATED..............November 05, 2025 at 17:33:38                  //
//                                                                           //
...
//***************************************************************************//
  ...
  control_unit_fsm
    control_unit_fsm_inst (
      .clk      ( part_PIs[2] ),      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
      .rst_n    ( part_PIs[7] ),      // pinName = rst_n;  tf = +SC  ; testOffset = 8.000000;  scanOffset = 0.000000;  
      .opcode   ({part_PIs[6]  ,      // pinName = opcode[3]; testOffset = 0.000000;  scanOffset = 0.000000;  
                  part_PIs[5]  ,      // pinName = opcode[2]; testOffset = 0.000000;  scanOffset = 0.000000;  
                  part_PIs[4]  ,      // pinName = opcode[1]; testOffset = 0.000000;  scanOffset = 0.000000;  
                  part_PIs[3]}),      // pinName = opcode[0]; testOffset = 0.000000;  scanOffset = 0.000000;  
      .RegWrite ( part_POs[8] ),      // pinName = RegWrite; 
      .ALUSrc   ( part_POs[3] ),      // pinName = ALUSrc; 
      .ALUOp    ({part_POs[2]  ,      // pinName = ALUOp[1]; 
                  part_POs[1]}),      // pinName = ALUOp[0]; 
      .MemRead  ( part_POs[5] ),      // pinName = MemRead; 
      .MemWrite ( part_POs[6] ),      // pinName = MemWrite; 
      .MemtoReg ( part_POs[7] ),      // pinName = MemtoReg; 
      .Branch   ( part_POs[4] ),      // pinName = Branch; 
      .SE       ( part_PIs[1] ),      // pinName = SE;  tf = +SE  ; testOffset = 0.000000;  scanOffset = 0.000000;  
      .scan_in  ( part_PIs[8] ),      // pinName = scan_in;  tf =  SI  ; testOffset = 0.000000;  scanOffset = 0.000000;  
...
//***************************************************************************//
  ...
  assign ( weak0, weak1 ) // Termination 
    part_POs [1] = global_term,     // pinName = ALUOp[0]; 
    part_POs [2] = global_term,     // pinName = ALUOp[1]; 
    part_POs [3] = global_term,     // pinName = ALUSrc; 
    part_POs [4] = global_term,     // pinName = Branch; 
    part_POs [5] = global_term,     // pinName = MemRead; 
    part_POs [6] = global_term,     // pinName = MemWrite; 
    part_POs [7] = global_term,     // pinName = MemtoReg; 
    part_POs [8] = global_term,     // pinName = RegWrite; 
...
//***************************************************************************//
  ...
  initial 
    begin 
      ...
      `ifdef sdf_annotate 
        `ifdef SDF_Minimum 
          $sdf_annotate ("default.sdf",control_unit_fsm_inst,,"sdf_Min.log","MINIMUM");
        `endif 
        `ifdef SDF_Maximum 
          $sdf_annotate ("default.sdf",control_unit_fsm_inst,,"sdf_Max.log","MAXIMUM");
        `endif 
        `ifdef SDF_Typical
          $sdf_annotate ("default.sdf",control_unit_fsm_inst,,"sdf_Typ.log","TYPICAL");
        `endif 
      `endif 
...
//***************************************************************************//
  ...
  task sim_setup; 
    begin 
      ...
      SERIALCYCLE = 0; 
      SEQNUM = 0; 
      name_POs [1] = "ALUOp[0]";      // pinName = ALUOp[0]; 
      name_POs [2] = "ALUOp[1]";      // pinName = ALUOp[1]; 
      name_POs [3] = "ALUSrc";      // pinName = ALUSrc; 
      name_POs [4] = "Branch";      // pinName = Branch; 
      name_POs [5] = "MemRead";      // pinName = MemRead; 
      name_POs [6] = "MemWrite";      // pinName = MemWrite; 
      name_POs [7] = "MemtoReg";      // pinName = MemtoReg; 
      name_POs [8] = "RegWrite";      // pinName = RegWrite; 
...
//***************************************************************************//
  ...
  task sim_vector_file; 
    begin 
      ...
      $display ( "\nINFO (TVE-189): Design:  control_unit_fsm   Test Mode:  FULLSCAN   InExperiment:  logic " ); 
  ...
  task test_cycle; 
    begin 
     ...
     #0.000000;        // 0.000000 ns;  From the start of the cycle.
      ...
      part_PIs [3] = stim_PIs [3];      // pinName = opcode[0]; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [4] = stim_PIs [4];      // pinName = opcode[1]; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [5] = stim_PIs [5];      // pinName = opcode[2]; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [6] = stim_PIs [6];      // pinName = opcode[3]; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [8] = stim_PIs [8];      // pinName = scan_in;  tf =  SI  ; testOffset = 0.000000;  scanOffset = 0.000000;  
     #8.000000;        // 8.000000 ns;  From the start of the cycle.
      ...
      part_PIs [7] = stim_PIs [7];      // pinName = rst_n;  tf = +SC  ; testOffset = 8.000000;  scanOffset = 0.000000;  
     #8.000000;        // 16.000000 ns;  From the start of the cycle.
      ...
      part_PIs [7] = stim_CIs [7];      // pinName = rst_n;  tf = +SC  ; testOffset = 8.000000;  scanOffset = 0.000000;  
     #56.000000;        // 72.000000 ns;  From the start of the cycle.
      ...
      for ( POnum = 1; POnum <= 9; POnum = POnum + 1 ) begin 
        if (( part_POs [ POnum ] !== meas_POs [ POnum ] ) & ( meas_POs [ POnum ] !== 1'bX )) begin 
          ...
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "control_unit_fsm", name_POs [ POnum ], PATTERN, -1, part_POs [ POnum ] ); 
          end  
        end  
  ...
  task Scan_Sequence_TM_1_SEQ_2_SOP_1; 
    begin 
      ...
      for ( SCANCYCLE = 1; SCANCYCLE <= MAX; SCANCYCLE = SCANCYCLE + 1 ) begin 
        ...
        SERIALCYCLE = SERIALCYCLE + 1; 
     #0.000000;        // 0.000000 ns;  From the start of the cycle.
        part_PIs [8] = stim_CR_1 [ 0 + SCANCYCLE ];      // pinName = scan_in;  tf =  SI  ; testOffset = 0.000000;  scanOffset = 0.000000;  
     #8.000000;        // 8.000000 ns;  From the start of the cycle.

        if (( part_POs [9] !== meas_OR_1 [ 0 + SCANCYCLE ] ) & ( meas_OR_1 [ 0 + SCANCYCLE ] !== 1'bX )) begin      // pinName = scan_out;  tf =  SO  ; 
          if ( test_num != failed_test_num )  begin 
            num_failed_tests = num_failed_tests + 1; 
            failed_test_num = test_num; 
          end  
          ...
          $display ( "           Expected: %0b   Simulated: %0b   Observe Register (OR) = 1;   Measured on Scan Cycle: %0d   Measured at SO: %0s   ", meas_OR_1 [ 0 + SCANCYCLE ], part_POs [9], SCANCYCLE, name_POs [9] ); 

          if (( failset ) & ( FAILSETID == 0 ))  failset_setup; 
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "control_unit_fsm", name_POs [9], PATTERN, SCANCYCLE, part_POs [9] ); 
          end  
        end  
        else  begin 
  ...
  task cmd_code; 
    begin 
      ...
      case ( CMD ) 
        ...
        200: begin 
          if ( rc_read > 0 )  begin 
            rc_read = $fscanf ( DATAID, "%b", stim_PIs [1:8] ); 
            if ( rc_read <= 0 )  bad_cmd_code; 
            line_number = line_number + 1; 
          end  
        end  
        ...
        201: begin 
          if ( rc_read > 0 )  begin 
            rc_read = $fscanf ( DATAID, "%b", stim_CIs [1:8] ); 
            if ( rc_read <= 0 )  bad_cmd_code; 
            line_number = line_number + 1; 
          end  
        end  
                ...
                if ( rc_read <= 0 )  bad_cmd_code; 
                  ...
                  case ( SCANNUM ) 
                    ...
                    1: begin 
                      ...
                      if ( rc_read > 0 )  begin 
                        rc_read = $fscanf ( DATAID, "%b", stim_CR_1 [1:4] ); 
                        if ( rc_read <= 0 )  bad_cmd_code; 
                        line_number = line_number + 1; 
                      end  
                    end  
                ...
                if ( rc_read <= 0 )  bad_cmd_code; 
                  ...
                  case ( SCANNUM ) 
                    ...
                    1: begin 
                      ...
                      if ( rc_read > 0 )  begin 
                        rc_read = $fscanf ( DATAID, "%b", meas_OR_1 [1:4] ); 
                        if ( rc_read <= 0 )  bad_cmd_code; 
                        line_number = line_number + 1; 
                      end  
                    end  
        ...
        904: begin 
          ...
          if ( rc_read > 0 )  begin 
            `ifdef MISCOMPAREDEBUG 
              if ( diag_debug ) begin 
                $processSimulationDebugFile ( DIAG_DEBUG_FILE, "control_unit_fsm_inst", eventID ); 
              end 
            `endif 
          end  
          else  bad_cmd_code; 
