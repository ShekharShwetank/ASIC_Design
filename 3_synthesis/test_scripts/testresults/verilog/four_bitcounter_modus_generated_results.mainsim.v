...
//***************************************************************************//
//                                                                           //
//  FILE CREATED..............September 09, 2025 at 16:29:59                 //
//                                                                           //
...
//***************************************************************************//
  ...
  four_bitcounter
    four_bitcounter_inst (
      .clk      ( part_PIs[2] ),      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
      .rst      ( part_PIs[3] ),      // pinName = rst; testOffset = 0.000000;  scanOffset = 0.000000;  
      .q        ({part_POs[4]  ,      // pinName = q[3]; 
                  part_POs[3]  ,      // pinName = q[2]; 
                  part_POs[2]  ,      // pinName = q[1]; 
                  part_POs[1]}),      // pinName = q[0]; 
      ...
      .scan_out ( part_POs[5] )     // pinName = scan_out;  tf =  SO  ; 
      );
...
//***************************************************************************//
  ...
  assign ( weak0, weak1 ) // Termination 
    part_POs [1] = global_term,     // pinName = q[0]; 
    part_POs [2] = global_term,     // pinName = q[1]; 
    part_POs [3] = global_term,     // pinName = q[2]; 
    part_POs [4] = global_term,     // pinName = q[3]; 
    part_POs [5] = global_term;      // pinName = scan_out;  tf =  SO  ; 
...
//***************************************************************************//
  ...
  initial 
    begin 
      ...
      `ifdef sdf_annotate 
        `ifdef SDF_Minimum 
          $sdf_annotate ("default.sdf",four_bitcounter_inst,,"sdf_Min.log","MINIMUM");
        `endif 
        `ifdef SDF_Maximum 
          $sdf_annotate ("default.sdf",four_bitcounter_inst,,"sdf_Max.log","MAXIMUM");
        `endif 
        `ifdef SDF_Typical
          $sdf_annotate ("default.sdf",four_bitcounter_inst,,"sdf_Typ.log","TYPICAL");
        `endif 
      `endif 
...
//***************************************************************************//
  ...
  task sim_setup; 
    begin 
      ...
      SERIALCYCLE = 0; 
      SEQNUM = 0; 
      name_POs [1] = "q[0]";      // pinName = q[0]; 
      name_POs [2] = "q[1]";      // pinName = q[1]; 
      name_POs [3] = "q[2]";      // pinName = q[2]; 
      name_POs [4] = "q[3]";      // pinName = q[3]; 
      name_POs [5] = "scan_out";      // pinName = scan_out;  tf =  SO  ; 
...
//***************************************************************************//
  ...
  task sim_vector_file; 
    begin 
      ...
      $display ( "\nINFO (TVE-189): Design:  four_bitcounter   Test Mode:  FULLSCAN   InExperiment:  logic " ); 
  ...
  task test_cycle; 
    begin 
     ...
     #0.000000;        // 0.000000 ns;  From the start of the cycle.
      ...
      part_PIs [3] = stim_PIs [3];      // pinName = rst; testOffset = 0.000000;  scanOffset = 0.000000;  
     ...
     #56.000000;        // 72.000000 ns;  From the start of the cycle.
      ...
      for ( POnum = 1; POnum <= 5; POnum = POnum + 1 ) begin 
        if (( part_POs [ POnum ] !== meas_POs [ POnum ] ) & ( meas_POs [ POnum ] !== 1'bX )) begin 
          ...
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "four_bitcounter", name_POs [ POnum ], PATTERN, -1, part_POs [ POnum ] ); 
          end  
        end  
  ...
  task Scan_Sequence_TM_1_SEQ_2_SOP_1; 
    begin 
     ...
     #8.000000;        // 8.000000 ns;  From the start of the cycle.
        ...
        if (( part_POs [5] !== meas_OR_1 [ 0 + SCANCYCLE ] ) & ( meas_OR_1 [ 0 + SCANCYCLE ] !== 1'bX )) begin      // pinName = scan_out;  tf =  SO  ; 
          if ( test_num != failed_test_num )  begin 
            num_failed_tests = num_failed_tests + 1; 
            failed_test_num = test_num; 
          end  
          ...
          $display ( "           Expected: %0b   Simulated: %0b   Observe Register (OR) = 1;   Measured on Scan Cycle: %0d   Measured at SO: %0s   ", meas_OR_1 [ 0 + SCANCYCLE ], part_POs [5], SCANCYCLE, name_POs [5] ); 

          if (( failset ) & ( FAILSETID == 0 ))  failset_setup; 
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "four_bitcounter", name_POs [5], PATTERN, SCANCYCLE, part_POs [5] ); 
          end  
        end  
        else  begin 
      ...
      case ( CMD ) 
        ...
        904: begin 
          ...
          if ( rc_read > 0 )  begin 
            `ifdef MISCOMPAREDEBUG 
              if ( diag_debug ) begin 
                $processSimulationDebugFile ( DIAG_DEBUG_FILE, "four_bitcounter_inst", eventID ); 
              end 
            `endif 
          end  
          else  bad_cmd_code; 
