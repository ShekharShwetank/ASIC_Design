...
//***************************************************************************//
//                                                                           //
//  FILE CREATED..............September 09, 2025 at 17:51:46                 //
//                                                                           //
...
//***************************************************************************//
  ...
  mux
    mux_inst (
      ...
      .clk          ( part_PIs [4] ),      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
      .rst          ( part_PIs [5] ),      // pinName = rst;  tf = -SC  ; testOffset = 8.000000;  scanOffset = 0.000000;  
      .s            ( part_PIs [6] ),      // pinName = s; testOffset = 0.000000;  scanOffset = 0.000000;  
...
//***************************************************************************//
  ...
  assign ( weak0, weak1 ) // Termination 
    ...
    part_POs [2] = global_term;      // pinName = y; 
...
//***************************************************************************//
  ...
  initial 
    begin 
      ...
      `ifdef sdf_annotate 
        `ifdef SDF_Minimum 
          $sdf_annotate ("default.sdf",mux_inst,,"sdf_Min.log","MINIMUM");
        `endif 
        `ifdef SDF_Maximum 
          $sdf_annotate ("default.sdf",mux_inst,,"sdf_Max.log","MAXIMUM");
        `endif 
        `ifdef SDF_Typical
          $sdf_annotate ("default.sdf",mux_inst,,"sdf_Typ.log","TYPICAL");
        `endif 
      `endif 
...
//***************************************************************************//
  ...
  task sim_vector_file; 
    begin 
      ...
      $display ( "\nINFO (TVE-189): Design:  mux   Test Mode:  FULLSCAN   InExperiment:  logic " ); 
  ...
  task test_cycle; 
    begin 
     ...
     #0.000000;        // 0.000000 ns;  From the start of the cycle.
      ...
      part_PIs [6] = stim_PIs [6];      // pinName = s; testOffset = 0.000000;  scanOffset = 0.000000;  
     ...
     #8.000000;        // 8.000000 ns;  From the start of the cycle.
      part_PIs [4] = stim_PIs [4];      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
      part_PIs [5] = stim_PIs [5];      // pinName = rst;  tf = -SC  ; testOffset = 8.000000;  scanOffset = 0.000000;  
     #8.000000;        // 16.000000 ns;  From the start of the cycle.
      part_PIs [4] = stim_CIs [4];      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
      part_PIs [5] = stim_CIs [5];      // pinName = rst;  tf = -SC  ; testOffset = 8.000000;  scanOffset = 0.000000;  
     #56.000000;        // 72.000000 ns;  From the start of the cycle.
      ...
      for ( POnum = 1; POnum <= 2; POnum = POnum + 1 ) begin 
        if (( part_POs [ POnum ] !== meas_POs [ POnum ] ) & ( meas_POs [ POnum ] !== 1'bX )) begin 
          ...
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "mux", name_POs [ POnum ], PATTERN, -1, part_POs [ POnum ] ); 
          end  
        end  
    ...
    begin 
     ...
     #8.000000;        // 8.000000 ns;  From the start of the cycle.
        ...
        if (( part_POs [1] !== meas_OR_1 [ 0 + SCANCYCLE ] ) & ( meas_OR_1 [ 0 + SCANCYCLE ] !== 1'bX )) begin      // pinName = scan_out;  tf =  SO  ; 
          ...
          if (( failset ) & ( FAILSETID == 0 ))  failset_setup; 
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "mux", name_POs [1], PATTERN, SCANCYCLE, part_POs [1] ); 
          end  
        end  
        else  begin 
     ...
     #8.000000;        // 16.000000 ns;  From the start of the cycle.
        part_PIs [4] = 1'b1;      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
     #8.000000;        // 24.000000 ns;  From the start of the cycle.
        part_PIs [4] = 1'b0;      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
      ...
      case ( CMD ) 
        ...
        904: begin 
          ...
          if ( rc_read > 0 )  begin 
            `ifdef MISCOMPAREDEBUG 
              if ( diag_debug ) begin 
                $processSimulationDebugFile ( DIAG_DEBUG_FILE, "mux_inst", eventID ); 
              end 
            `endif 
          end  
          else  bad_cmd_code; 
