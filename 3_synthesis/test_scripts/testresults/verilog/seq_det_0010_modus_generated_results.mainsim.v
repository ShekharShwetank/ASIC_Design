...
//***************************************************************************//
//                                                                           //
//  FILE CREATED..............September 09, 2025 at 17:57:51                 //
//                                                                           //
...
//***************************************************************************//
  ...
  seq_det_0010
    seq_det_0010_inst (
      ...
      .rst_n    ( part_PIs[4] ),      // pinName = rst_n;  tf = +SC  ; testOffset = 8.000000;  scanOffset = 0.000000;  
      .in       ( part_PIs[3] ),      // pinName = in; testOffset = 0.000000;  scanOffset = 0.000000;  
      .out      ( part_POs[1] ),      // pinName = out; 
...
//***************************************************************************//
  ...
  assign ( weak0, weak1 ) // Termination 
    part_POs [1] = global_term,     // pinName = out; 
...
//***************************************************************************//
  ...
  initial 
    begin 
      ...
      `ifdef sdf_annotate 
        `ifdef SDF_Minimum 
          $sdf_annotate ("default.sdf",seq_det_0010_inst,,"sdf_Min.log","MINIMUM");
        `endif 
        `ifdef SDF_Maximum 
          $sdf_annotate ("default.sdf",seq_det_0010_inst,,"sdf_Max.log","MAXIMUM");
        `endif 
        `ifdef SDF_Typical
          $sdf_annotate ("default.sdf",seq_det_0010_inst,,"sdf_Typ.log","TYPICAL");
        `endif 
      `endif 
...
//***************************************************************************//
  ...
  task sim_vector_file; 
    begin 
      ...
      $display ( "\nINFO (TVE-189): Design:  seq_det_0010   Test Mode:  FULLSCAN   InExperiment:  logic " ); 
  ...
  task test_cycle; 
    begin 
     ...
     #0.000000;        // 0.000000 ns;  From the start of the cycle.
      ...
      part_PIs [3] = stim_PIs [3];      // pinName = in; testOffset = 0.000000;  scanOffset = 0.000000;  
     ...
     #8.000000;        // 8.000000 ns;  From the start of the cycle.
      ...
      part_PIs [4] = stim_PIs [4];      // pinName = rst_n;  tf = +SC  ; testOffset = 8.000000;  scanOffset = 0.000000;  
     #8.000000;        // 16.000000 ns;  From the start of the cycle.
      ...
      part_PIs [4] = stim_CIs [4];      // pinName = rst_n;  tf = +SC  ; testOffset = 8.000000;  scanOffset = 0.000000;  
     #56.000000;        // 72.000000 ns;  From the start of the cycle.
      ...
      for ( POnum = 1; POnum <= 2; POnum = POnum + 1 ) begin 
        if (( part_POs [ POnum ] !== meas_POs [ POnum ] ) & ( meas_POs [ POnum ] !== 1'bX )) begin 
          ...
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "seq_det_0010", name_POs [ POnum ], PATTERN, -1, part_POs [ POnum ] ); 
          end  
        end  
    ...
    begin 
     ...
     #8.000000;        // 8.000000 ns;  From the start of the cycle.
        ...
        if (( part_POs [2] !== meas_OR_1 [ 0 + SCANCYCLE ] ) & ( meas_OR_1 [ 0 + SCANCYCLE ] !== 1'bX )) begin      // pinName = scan_out;  tf =  SO  ; 
          ...
          if (( failset ) & ( FAILSETID == 0 ))  failset_setup; 
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "seq_det_0010", name_POs [2], PATTERN, SCANCYCLE, part_POs [2] ); 
          end  
        end  
        else  begin 
      ...
      case ( CMD ) 
                ...
                if ( rc_read <= 0 )  bad_cmd_code; 
                  ...
                  case ( SCANNUM ) 
                    ...
                    1: begin 
                      ...
                      if ( rc_read > 0 )  begin 
                        rc_read = $fscanf ( DATAID, "%b", stim_CR_1 [1:3] ); 
                        if ( rc_read <= 0 )  bad_cmd_code; 
                        line_number = line_number + 1; 
                      end  
                    end  
                ...
                if ( rc_read <= 0 )  bad_cmd_code; 
                  ...
                  case ( SCANNUM ) 
                    ...
                    1: begin 
                      ...
                      if ( rc_read > 0 )  begin 
                        rc_read = $fscanf ( DATAID, "%b", meas_OR_1 [1:3] ); 
                        if ( rc_read <= 0 )  bad_cmd_code; 
                        line_number = line_number + 1; 
                      end  
                    end  
        ...
        904: begin 
          ...
          if ( rc_read > 0 )  begin 
            `ifdef MISCOMPAREDEBUG 
              if ( diag_debug ) begin 
                $processSimulationDebugFile ( DIAG_DEBUG_FILE, "seq_det_0010_inst", eventID ); 
              end 
            `endif 
          end  
          else  bad_cmd_code; 
