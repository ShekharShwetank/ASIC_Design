...
//***************************************************************************//
//                                                                           //
//  FILE CREATED..............September 09, 2025 at 17:55:17                 //
//                                                                           //
...
//***************************************************************************//
  ...
  decoder
    decoder_inst (
      .y        ({part_POs[9]  ,      // pinName = y[7]; 
                  part_POs[8]  ,      // pinName = y[6]; 
                  part_POs[7]  ,      // pinName = y[5]; 
                  part_POs[6]  ,      // pinName = y[4]; 
                  part_POs[5]  ,      // pinName = y[3]; 
                  part_POs[4]  ,      // pinName = y[2]; 
                  part_POs[3]  ,      // pinName = y[1]; 
                  part_POs[2]}),      // pinName = y[0]; 
      .a        ({part_PIs[4]  ,      // pinName = a[2]; testOffset = 0.000000;  scanOffset = 0.000000;  
                  part_PIs[3]  ,      // pinName = a[1]; testOffset = 0.000000;  scanOffset = 0.000000;  
                  part_PIs[2]}),      // pinName = a[0]; testOffset = 0.000000;  scanOffset = 0.000000;  
      .clk      ( part_PIs[5] ),      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
      .rst      ( part_PIs[6] ),      // pinName = rst; testOffset = 0.000000;  scanOffset = 0.000000;  
      .SE       ( part_PIs[1] ),      // pinName = SE;  tf = +SE  ; testOffset = 0.000000;  scanOffset = 0.000000;  
      .scan_in  ( part_PIs[7] ),      // pinName = scan_in;  tf =  SI  ; testOffset = 0.000000;  scanOffset = 0.000000;  
      .scan_out ( part_POs[1] )     // pinName = scan_out;  tf =  SO  ; 
      );
...
//***************************************************************************//
  ...
  assign ( weak0, weak1 ) // Termination 
    part_POs [1] = global_term,     // pinName = scan_out;  tf =  SO  ; 
    part_POs [2] = global_term,     // pinName = y[0]; 
    part_POs [3] = global_term,     // pinName = y[1]; 
    part_POs [4] = global_term,     // pinName = y[2]; 
    part_POs [5] = global_term,     // pinName = y[3]; 
    part_POs [6] = global_term,     // pinName = y[4]; 
    part_POs [7] = global_term,     // pinName = y[5]; 
    part_POs [8] = global_term,     // pinName = y[6]; 
    part_POs [9] = global_term;      // pinName = y[7]; 
...
//***************************************************************************//
  ...
  initial 
    begin 
      ...
      `ifdef sdf_annotate 
        `ifdef SDF_Minimum 
          $sdf_annotate ("default.sdf",decoder_inst,,"sdf_Min.log","MINIMUM");
        `endif 
        `ifdef SDF_Maximum 
          $sdf_annotate ("default.sdf",decoder_inst,,"sdf_Max.log","MAXIMUM");
        `endif 
        `ifdef SDF_Typical
          $sdf_annotate ("default.sdf",decoder_inst,,"sdf_Typ.log","TYPICAL");
        `endif 
      `endif 
...
//***************************************************************************//
  ...
  task sim_setup; 
    begin 
      ...
      SERIALCYCLE = 0; 
      SEQNUM = 0; 
      name_POs [1] = "scan_out";      // pinName = scan_out;  tf =  SO  ; 
      name_POs [2] = "y[0]";      // pinName = y[0]; 
      name_POs [3] = "y[1]";      // pinName = y[1]; 
      name_POs [4] = "y[2]";      // pinName = y[2]; 
      name_POs [5] = "y[3]";      // pinName = y[3]; 
      name_POs [6] = "y[4]";      // pinName = y[4]; 
      name_POs [7] = "y[5]";      // pinName = y[5]; 
      name_POs [8] = "y[6]";      // pinName = y[6]; 
      name_POs [9] = "y[7]";      // pinName = y[7]; 
...
//***************************************************************************//
  ...
  task sim_vector_file; 
    begin 
      ...
      $display ( "\nINFO (TVE-189): Design:  decoder   Test Mode:  FULLSCAN   InExperiment:  logic " ); 
  ...
  task test_cycle; 
    begin 
     ...
     #0.000000;        // 0.000000 ns;  From the start of the cycle.
      ...
      part_PIs [3] = stim_PIs [3];      // pinName = a[1]; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [4] = stim_PIs [4];      // pinName = a[2]; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [6] = stim_PIs [6];      // pinName = rst; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [7] = stim_PIs [7];      // pinName = scan_in;  tf =  SI  ; testOffset = 0.000000;  scanOffset = 0.000000;  
     #8.000000;        // 8.000000 ns;  From the start of the cycle.
      part_PIs [5] = stim_PIs [5];      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
     #8.000000;        // 16.000000 ns;  From the start of the cycle.
      part_PIs [5] = stim_CIs [5];      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
     #56.000000;        // 72.000000 ns;  From the start of the cycle.
      ...
      for ( POnum = 1; POnum <= 9; POnum = POnum + 1 ) begin 
        if (( part_POs [ POnum ] !== meas_POs [ POnum ] ) & ( meas_POs [ POnum ] !== 1'bX )) begin 
          ...
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "decoder", name_POs [ POnum ], PATTERN, -1, part_POs [ POnum ] ); 
          end  
        end  
  ...
  task Scan_Sequence_TM_1_SEQ_2_SOP_1; 
    begin 
      ...
      for ( SCANCYCLE = 1; SCANCYCLE <= MAX; SCANCYCLE = SCANCYCLE + 1 ) begin 
        ...
        SERIALCYCLE = SERIALCYCLE + 1; 
     #0.000000;        // 0.000000 ns;  From the start of the cycle.
        part_PIs [7] = stim_CR_1 [ 0 + SCANCYCLE ];      // pinName = scan_in;  tf =  SI  ; testOffset = 0.000000;  scanOffset = 0.000000;  
     #8.000000;        // 8.000000 ns;  From the start of the cycle.

        if (( part_POs [1] !== meas_OR_1 [ 0 + SCANCYCLE ] ) & ( meas_OR_1 [ 0 + SCANCYCLE ] !== 1'bX )) begin      // pinName = scan_out;  tf =  SO  ; 
          if ( test_num != failed_test_num )  begin 
            num_failed_tests = num_failed_tests + 1; 
            failed_test_num = test_num; 
          end  
          ...
          if (( failset ) & ( FAILSETID == 0 ))  failset_setup; 
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "decoder", name_POs [1], PATTERN, SCANCYCLE, part_POs [1] ); 
          end  
        end  
        else  begin 
     ...
     #8.000000;        // 16.000000 ns;  From the start of the cycle.
        part_PIs [5] = 1'b1;      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
     #8.000000;        // 24.000000 ns;  From the start of the cycle.
        part_PIs [5] = 1'b0;      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
  ...
  task cmd_code; 
    begin 
      ...
      case ( CMD ) 
        ...
        200: begin 
          if ( rc_read > 0 )  begin 
            rc_read = $fscanf ( DATAID, "%b", stim_PIs [1:7] ); 
            if ( rc_read <= 0 )  bad_cmd_code; 
            line_number = line_number + 1; 
          end  
        end  
        ...
        201: begin 
          if ( rc_read > 0 )  begin 
            rc_read = $fscanf ( DATAID, "%b", stim_CIs [1:7] ); 
            if ( rc_read <= 0 )  bad_cmd_code; 
            line_number = line_number + 1; 
          end  
        end  
        ...
        904: begin 
          ...
          if ( rc_read > 0 )  begin 
            `ifdef MISCOMPAREDEBUG 
              if ( diag_debug ) begin 
                $processSimulationDebugFile ( DIAG_DEBUG_FILE, "decoder_inst", eventID ); 
              end 
            `endif 
          end  
          else  bad_cmd_code; 
