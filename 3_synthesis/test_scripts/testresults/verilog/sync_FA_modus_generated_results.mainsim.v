...
//***************************************************************************//
//                                                                           //
//  FILE CREATED..............September 09, 2025 at 16:47:29                 //
//                                                                           //
...
//***************************************************************************//
  ...
  sync_FA
    sync_FA_inst (
      .cout         ( part_POs [1] ),      // pinName = cout; 
      .scan_out     ( part_POs [2] ),      // pinName = scan_out;  tf =  SO  ; 
      .sum          ( part_POs [3] ),      // pinName = sum; 
      .SE           ( part_PIs [1] ),      // pinName = SE;  tf = +SE  ; testOffset = 0.000000;  scanOffset = 0.000000;  
      .a            ( part_PIs [2] ),      // pinName = a; testOffset = 0.000000;  scanOffset = 0.000000;  
      .b            ( part_PIs [3] ),      // pinName = b; testOffset = 0.000000;  scanOffset = 0.000000;  
      .cin          ( part_PIs [4] ),      // pinName = cin; testOffset = 0.000000;  scanOffset = 0.000000;  
      .clk          ( part_PIs [5] ),      // pinName = clk;  tf = -ES  ; testOffset = 8.000000;  scanOffset = 16.000000;  
      .scan_in      ( part_PIs [6] ));      // pinName = scan_in;  tf =  SI  ; testOffset = 0.000000;  scanOffset = 0.000000;  
...
//***************************************************************************//
  ...
  assign ( weak0, weak1 ) // Termination 
    part_POs [1] = global_term,     // pinName = cout; 
    part_POs [2] = global_term,     // pinName = scan_out;  tf =  SO  ; 
    part_POs [3] = global_term;      // pinName = sum; 
...
//***************************************************************************//
  ...
  initial 
    begin 
      ...
      `ifdef sdf_annotate 
        `ifdef SDF_Minimum 
          $sdf_annotate ("default.sdf",sync_FA_inst,,"sdf_Min.log","MINIMUM");
        `endif 
        `ifdef SDF_Maximum 
          $sdf_annotate ("default.sdf",sync_FA_inst,,"sdf_Max.log","MAXIMUM");
        `endif 
        `ifdef SDF_Typical
          $sdf_annotate ("default.sdf",sync_FA_inst,,"sdf_Typ.log","TYPICAL");
        `endif 
      `endif 
...
//***************************************************************************//
  ...
  task sim_setup; 
    begin 
      ...
      SERIALCYCLE = 0; 
      SEQNUM = 0; 
      name_POs [1] = "cout";      // pinName = cout; 
      name_POs [2] = "scan_out";      // pinName = scan_out;  tf =  SO  ; 
      name_POs [3] = "sum";      // pinName = sum; 
...
//***************************************************************************//
  ...
  task sim_vector_file; 
    begin 
      ...
      $display ( "\nINFO (TVE-189): Design:  sync_FA   Test Mode:  FULLSCAN   InExperiment:  logic " ); 
  ...
  task test_cycle; 
    begin 
     ...
     #0.000000;        // 0.000000 ns;  From the start of the cycle.
      ...
      part_PIs [2] = stim_PIs [2];      // pinName = a; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [3] = stim_PIs [3];      // pinName = b; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [4] = stim_PIs [4];      // pinName = cin; testOffset = 0.000000;  scanOffset = 0.000000;  
      part_PIs [6] = stim_PIs [6];      // pinName = scan_in;  tf =  SI  ; testOffset = 0.000000;  scanOffset = 0.000000;  
     ...
     #64.000000;        // 72.000000 ns;  From the start of the cycle.
      ...
      for ( POnum = 1; POnum <= 3; POnum = POnum + 1 ) begin 
        if (( part_POs [ POnum ] !== meas_POs [ POnum ] ) & ( meas_POs [ POnum ] !== 1'bX )) begin 
          ...
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "sync_FA", name_POs [ POnum ], PATTERN, -1, part_POs [ POnum ] ); 
          end  
        end  
  ...
  task Scan_Sequence_TM_1_SEQ_2_SOP_1; 
    begin 
      ...
      for ( SCANCYCLE = 1; SCANCYCLE <= MAX; SCANCYCLE = SCANCYCLE + 1 ) begin 
        ...
        SERIALCYCLE = SERIALCYCLE + 1; 
     #0.000000;        // 0.000000 ns;  From the start of the cycle.
        part_PIs [6] = stim_CR_1 [ 0 + SCANCYCLE ];      // pinName = scan_in;  tf =  SI  ; testOffset = 0.000000;  scanOffset = 0.000000;  
     #8.000000;        // 8.000000 ns;  From the start of the cycle.

        if (( part_POs [2] !== meas_OR_1 [ 0 + SCANCYCLE ] ) & ( meas_OR_1 [ 0 + SCANCYCLE ] !== 1'bX )) begin      // pinName = scan_out;  tf =  SO  ; 
          if ( test_num != failed_test_num )  begin 
            num_failed_tests = num_failed_tests + 1; 
            failed_test_num = test_num; 
          end  
          ...
          $display ( "           Expected: %0b   Simulated: %0b   Observe Register (OR) = 1;   Measured on Scan Cycle: %0d   Measured at SO: %0s   ", meas_OR_1 [ 0 + SCANCYCLE ], part_POs [2], SCANCYCLE, name_POs [2] ); 

          if (( failset ) & ( FAILSETID == 0 ))  failset_setup; 
          if ( FAILSETID ) begin 
            $fdisplay ( FAILSETID, " Chip %0s pad %0s pattern %0s position %0d value %0b ", "sync_FA", name_POs [2], PATTERN, SCANCYCLE, part_POs [2] ); 
          end  
        end  
        else  begin 
  ...
  task cmd_code; 
    begin 
      ...
      case ( CMD ) 
        ...
        200: begin 
          if ( rc_read > 0 )  begin 
            rc_read = $fscanf ( DATAID, "%b", stim_PIs [1:6] ); 
            if ( rc_read <= 0 )  bad_cmd_code; 
            line_number = line_number + 1; 
          end  
        end  
        ...
        201: begin 
          if ( rc_read > 0 )  begin 
            rc_read = $fscanf ( DATAID, "%b", stim_CIs [1:6] ); 
            if ( rc_read <= 0 )  bad_cmd_code; 
            line_number = line_number + 1; 
          end  
        end  
        ...
        202: begin 
          if ( rc_read > 0 )  begin 
            rc_read = $fscanf ( DATAID, "%b", meas_POs [1:3] ); 
            if ( rc_read <= 0 )  bad_cmd_code; 
            line_number = line_number + 1; 
          end  
        end  
                ...
                if ( rc_read <= 0 )  bad_cmd_code; 
                  ...
                  case ( SCANNUM ) 
                    ...
                    1: begin 
                      ...
                      if ( rc_read > 0 )  begin 
                        rc_read = $fscanf ( DATAID, "%b", stim_CR_1 [1:2] ); 
                        if ( rc_read <= 0 )  bad_cmd_code; 
                        line_number = line_number + 1; 
                      end  
                    end  
                ...
                if ( rc_read <= 0 )  bad_cmd_code; 
                  ...
                  case ( SCANNUM ) 
                    ...
                    1: begin 
                      ...
                      if ( rc_read > 0 )  begin 
                        rc_read = $fscanf ( DATAID, "%b", meas_OR_1 [1:2] ); 
                        if ( rc_read <= 0 )  bad_cmd_code; 
                        line_number = line_number + 1; 
                      end  
                    end  
        ...
        904: begin 
          ...
          if ( rc_read > 0 )  begin 
            `ifdef MISCOMPAREDEBUG 
              if ( diag_debug ) begin 
                $processSimulationDebugFile ( DIAG_DEBUG_FILE, "sync_FA_inst", eventID ); 
              end 
            `endif 
          end  
          else  bad_cmd_code; 
